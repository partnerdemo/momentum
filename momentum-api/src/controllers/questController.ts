...
import { addPointsRaw } from '../services/pointsService';
...
export const approveQuest = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
        ...
        const io = req.app.get('io');
        const { updatedProfile } = await addPointsRaw(io, householdId!, memberId, quest.pointsValue);
        const newPointsTotal = updatedProfile?.pointsTotal || 0;

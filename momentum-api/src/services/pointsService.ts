...
/**
 * Add points raw to a member profile, wrapping it in a transaction and syncing to linked households,
 * but completely bypassing the streak multiplier (intended for Quests or other non-assigned tasks).
 */
export const addPointsRaw = async (
    io: any,
    householdId: Types.ObjectId | string,
    memberId: Types.ObjectId | string, // The memberProfile ID
    points: number
    ...
    try {
        ...
        // 1. Update Points (primary household)
        memberProfile.pointsTotal = (memberProfile.pointsTotal || 0) + points;
        await household.save({ session });

        // 2. Sync to Linked Households (within the same transaction)
        if (memberProfile.familyMemberId) {
            await syncPointsToLinkedHouseholds(io, memberProfile.familyMemberId, householdId, points, session);
        }

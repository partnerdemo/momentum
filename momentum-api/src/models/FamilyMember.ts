...
const FamilyMemberSchema = new Schema<IFamilyMember>(
  ...
  },
  {
    ...
    // Strip sensitive fields whenever the document is serialized — protects
    // against controllers that build response payloads via `.toObject()` /
    // `.toJSON()` and would otherwise leak the bcrypt hash for password or PIN.
    toObject: {
      transform: (_doc, ret) => {
        delete ret.password;
        delete ret.pin;
        delete ret.googleCalendar;
        return ret;
      },
    },
    toJSON: {
...
// Compare PIN method (secure, quiet in production)
FamilyMemberSchema.methods.comparePin = async function (candidatePin: string): Promise<boolean> {
  ...
  if (!this.pin) {
    ...
    return bcrypt.compare(pinStr, user.pin);
  }

  return bcrypt.compare(pinStr, this.pin);
};
